...
class NavigateToGoal(Node):
    ...
    def __init__(self):
        ...
        if self.planner is not None:
            # Use transient local QoS to receive latched map from map_server
            map_qos = QoSProfile(
                depth=10,
    ...
    def rotation_callback(self):
        ...
        dx = self.goal_x - self.robot_pose.pose.position.x
        dy = self.goal_y - self.robot_pose.pose.position.y
    ...
    def generate_and_publish_path(self):
        ...
        if self.planner is not None:
            ...
            path_waypoints = self.planner.plan_path(x_start, y_start, self.goal_x, self.goal_y, start_yaw=yaw_start)
            ...
            if path_waypoints is None or len(path_waypoints) == 0:
                ...
                # Path is already simplified and smoothed by A* planner
                self.get_logger().info(f'Path already smoothed by A* planner')

                # Densify path for better tracking (reduced density to prevent jerking)
                # Raw A* path already has many waypoints from grid, don't over-densify
                path_waypoints = self.densify_waypoints(path_waypoints, points_per_meter=3)
                self.get_logger().info(f'Densified path to {len(path_waypoints)} waypoints for smooth tracking')
    ...
    def densify_waypoints(self, waypoints, points_per_meter=20):
        """
        Interpolate waypoints to create a denser path for better tracking.

        Args:
            waypoints: List of (x, y, yaw) tuples (yaw can be None)
            points_per_meter: Target density of waypoints

        Returns:
            List of interpolated (x, y, yaw) tuples
        """
        if len(waypoints) < 2:
            return waypoints
    ...
    def waypoints_to_path(self, waypoints):
        ...
        Args:
            waypoints: List of (x, y, yaw) tuples where yaw can be None
        ...
        for i, waypoint in enumerate(waypoints):
            ...
            if explicit_yaw is not None:
                yaw = explicit_yaw
                # Debug: log rotation waypoints
                if i < 10:  # Only log first few to avoid spam
                    self.get_logger().debug(f'  Rotation waypoint {i}: yaw={math.degrees(yaw):.1f}°')
